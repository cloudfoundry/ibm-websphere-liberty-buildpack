...
| -------------  |:-------------:| --------- |
| Pushed a wrongly packaged server please use 'server package --include=user' to package a server | You attempted to push a server that contains binaries.   | Please package the server without the binaries.<br>Run the server package command with the '--include=usr' option to achieve this.     |
