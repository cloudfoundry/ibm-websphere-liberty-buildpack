...
describe LibertyBuildpack::Util::Cache::InternetAvailability do
  ...
  it 'should raise error if remote downloads are wrongly configured' do
    allow(LibertyBuildpack::Util::ConfigurationUtils).to receive(:load).with('cache').and_return('remote_downloads' => 'x')
