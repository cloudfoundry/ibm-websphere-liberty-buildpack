...
require 'liberty_buildpack/framework/dynatrace_appmon_agent'
require 'liberty_buildpack/container/common_paths'

module LibertyBuildpack::Framework

  describe 'DynatraceAppmonAgent' do
    include_context 'component_helper' # component context

    # test data
    let(:dynatrace_home) { '.dynatrace_appmon_agent' } # the expected staged dynatrace agent directory
    let(:application_cache) { double('ApplicationCache') }
    let(:version) { '6.2.0_1238' }
    let(:jar_name) { 'dynatrace-agent-unix.jar' }
    let(:detect_string) { "dynatrace-appmon-agent-#{version}" }
    ...
    before do |example|
      ...
      application_cache.stub(:get).with(index_uri).and_yield(File.open('spec/fixtures/stub-dynatrace-appmon-agent.jar'))
    end

    describe 'configuration' do
      it 'must have v6.2.0_1238 as the configured version' do
        configuration = YAML.load_file(File.expand_path('../../../config/dynatraceappmonagent.yml', File.dirname(__FILE__)))
    ...
    describe 'detect',
      ...
      subject(:detected) { DynatraceAppmonAgent.new(context).detect }
    ...
    describe 'compile',
      ...
      subject(:compiled) do
        dynatrace = DynatraceAppmonAgent.new(context)
        dynatrace.detect
        dynatrace.compile
      end
      ...
      describe 'download agent jar based on index.yml information' do
        it 'should download the agent with a matching key and jar version' do
          expect { compiled }.to output(%r{Downloading Dynatrace Appmon Agent #{version} from https://downloadsite/dynatrace/dynatrace-agent-unix.jar}).to_stdout
        ...
        it 'should raise an error with original exception if the jar could not be downloaded',
          ...
          expect { compiled }.to raise_error(/Unable to download the Dynatrace Appmon Agent jar..+underlying download error/)
        end
      end
    end # end compile
    ...
    describe 'release',
      ...
      it 'should return command line options for a valid service in a default container' do
        ...
        excpect(released[1]).to eq('-Xshare:off')
      end
