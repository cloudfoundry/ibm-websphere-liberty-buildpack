...
describe LibertyBuildpack::BuildpackVersion do
  ...
  it 'should exclude remote string when remote and hash values from config/version.yml are empty',
     configuration: { 'hash' => '', 'remote' => '', 'version' => 'test-version' } do
    expect(buildpack_version.to_s).to eq('test-version')
  end

  it 'should include remote string when remote and hash values from config/version.yml are missing',
