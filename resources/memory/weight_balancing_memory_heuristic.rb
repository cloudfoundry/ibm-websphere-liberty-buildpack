...
require_relative 'stack_memory_bucket'
...
class WeightBalancingMemoryHeuristic
  ...
  def issue_memory_wastage_warning(buckets)
    ...
    puts "The allocated Java memory sizes total #{total_size} which is less than #{TOTAL_MEMORY_WARNING_FACTOR} of the available memory, so configured Java memory sizes may be too small or available memory may be too large"
  end
  ...
  def check_close_to_default(type, bucket, total_weighting)
    ...
    if default_size > 0
      ...
      puts "factor for memory size #{type} is #{factor}"
    end

    return unless (default_size == 0 && actual_size == 0) || (factor && (factor < CLOSE_TO_DEFAULT_FACTOR))

    puts "The computed value #{actual_size} of memory size #{type} is close to the default value #{default_size}. Consider taking the default."
  end

  # This outputs the JVM settings to be written, such as:
  # -Xmx372053K
  # -Xms372053K
